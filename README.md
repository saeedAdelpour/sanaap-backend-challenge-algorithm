...
After cloning the repository, open a terminal in the project root and set up the environment:

```sh
uv sync
```

This creates a local `.venv` and installs the project using `uv.lock`. You do not need to activate the environment when using `uv run`.
